...
export class DashboardRepository {
  ...
  async getKPIs() {
    ...
    return {
      ...
      totalRevenue: Number(orders._sum.total ?? new Prisma.Decimal(0)),

      averageTicket: Number(orders._avg.total ?? new Prisma.Decimal(0)),
      ...
      approvalRate:
        totalOrders === 0
          ? 0
          : Number(((approvedOrders / totalOrders) * 100).toFixed(1)),
      ...
      averageProcessingTime: Number(averageProcessingTime.toFixed(1)),
    };
  }
  ...
  async getStatusSummary() {
    ...
    return {
      pending: status.find((s) => s.status === 'PENDING')?._count ?? 0,

      processing:
        status.find((s) => s.status === 'PROCESSING_PAYMENT')?._count ?? 0,

      approved: status.find((s) => s.status === 'APPROVED')?._count ?? 0,

      rejected: status.find((s) => s.status === 'REJECTED')?._count ?? 0,
    };
  }
  ...
  async getRevenueTimeline() {
    ...
    for (const order of orders) {
      ...
      grouped.set(day, (grouped.get(day) ?? 0) + Number(order.total));
    }
