...
import { RABBITMQ_QUEUE, RABBITMQ_DLQ } from './rabbitmq.constants';
...
export class RabbitMQService implements OnModuleInit, OnModuleDestroy {
  ...
  async onModuleInit() {
    ...
    this.logger.log(`DLQ "${RABBITMQ_DLQ}" is ready`);
  }
  ...
  async reprocessFromDlq(): Promise<boolean> {
    const message = await this.channel.get(RABBITMQ_DLQ, {
      noAck: false,
    });

    if (!message) {
      this.logger.log('No messages available in DLQ.');
    ...
    try {
      ...
      this.channel.sendToQueue(RABBITMQ_QUEUE, message.content, {
        persistent: true,
        headers,
      });
